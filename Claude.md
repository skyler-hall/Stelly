...
The project also doubles as Skyler's vehicle for building real, hands-on engineering skill.
...
Edit on the Mac in VS Code, push to GitHub, pull on the Pi. Three machines stay in sync through GitHub: Mac, Raspberry Pi, Windows PC. See @SYNC.md for the full multi-machine setup. `.env` is specific to each machine and never syncs through git.
